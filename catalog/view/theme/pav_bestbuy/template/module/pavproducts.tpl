...
<div class="box pav-categoryproducts">
	...
	<h3 class="box-heading"><span><?php echo $this->language->get( 'heading_title' ); ?></span></h3>
				...
				<div class="tab-nav span3">
						<ul class="h-tabs" id="producttabs<?php echo $id;?>">
							<?php foreach( $tabs as $tab => $category ) { 
								...
								$icon = $this->model_tool_image->resize( $category['image'], 44, 40 ); 
							?>
								 <li>
								...
								<?php foreach( $tproducts as $i => $product ) { ?>
									...
									<?php } ?>
									      ...
									      <div class="product-meta">
										      ...
										      <div class="rating"><img src="catalog/view/theme/<?php echo $themeName;?>/image/stars-<?php echo $product['rating']; ?>.png" alt="<?php echo $product['reviews']; ?>" /></div>
										      <?php } ?>
											...
											<div class="group-action">
												 <div class="cart"><input type="button" value="<?php echo $button_cart; ?>" onclick="addToCart('<?php echo $product['product_id']; ?>');" class="button" /></div>
												  <div class="wishlist">
												  	<a onclick="addToWishList('<?php echo $product['product_id']; ?>');" title="<?php echo $this->language->get("button_wishlist"); ?>" ><?php echo $this->language->get("button_wishlist"); ?></a>
												  </div>
												  <div class="compare">
												  	<a onclick="addToCompare('<?php echo $product['product_id']; ?>');" title="<?php echo $this->language->get("button_compare"); ?>" ><?php echo $this->language->get("button_compare"); ?></a>
												  </div>
											 </div>
